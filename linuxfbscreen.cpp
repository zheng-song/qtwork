...
/*static void Change_Rects(QRect &sourceRects,bool clockWise)
{
    QRect targetRects = sourceRects;
    if (!clockWise)
    {
        targetRects.x() = sourceRects.y();
        targetRects.y() = sourceRects
    }
...
//void drawImage(const QRectF &targetRect, const QImage &image, const QRectF &sourceRect,Qt::ImageConversionFlags flags = Qt::AutoColor);
                mBlitter->drawImage(rects[i], *mScreenImage, rects[i]);
                // My_Copy_Image(mScreenImage,&mFbScreenImage,rects[i]);
                break;

            case 1:
               /* QRect sourceRect = rects[i];
                Change_Rects(rects[i],true);
                mBlitter->drawImage(rects[i], *mScreenImage,sourceRect);*/
